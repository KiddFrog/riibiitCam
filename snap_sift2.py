...
# Define a region of interest (ROI) around the subject in each image
roi1 = image1_rgb[100:500, 100:500]
roi2 = image2_rgb[100:500, 100:500]
roi3 = image3_rgb[100:500, 100:500]
roi4 = image4_rgb[100:500, 100:500]

# Initialize SIFT detector
sift = cv2.SIFT_create()

# Find the keypoints and descriptors with SIFT for the ROI
kp1, des1 = sift.detectAndCompute(roi1, None)
kp2, des2 = sift.detectAndCompute(roi2, None)  # The baseline image
kp3, des3 = sift.detectAndCompute(roi3, None)
kp4, des4 = sift.detectAndCompute(roi4, None)
...
if len(good_matches1to2) < 4 or len(good_matches3to2) < 4 or len(good_matches4to2) < 4:
    ...
    # Wait until all aligned images are available
    max_attempts = 10
    current_attempt = 0
    while current_attempt < max_attempts and not all(os.path.exists(path) for path in aligned_image_paths):
        time.sleep(2)  # Wait for 2 seconds before checking again
        current_attempt += 1

    if all(os.path.exists(path) for path in aligned_image_paths):
        ...
        print("Failed to generate GIF. Some aligned images are missing.")
